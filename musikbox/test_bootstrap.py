...
def test_create_app_uses_fake_analyzer_when_essentia_unavailable(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("MUSIKBOX_MUSIC_DIR", str(tmp_path / "music"))
    ...
    # Ensure essentia cannot be imported so bootstrap falls back to FakeAnalyzer
    real_import = builtins.__import__

    def _block_essentia(name: str, *args: object, **kwargs: object) -> object:
        if name == "essentia":
            raise ImportError("blocked for test")
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", _block_essentia)

    app = create_app()

    # The analyzer is injected into AnalysisService as _analyzer
